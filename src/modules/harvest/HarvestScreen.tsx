/**
 * HarvestScreen
 * ================
 * bkz. Sprint 8.2. `MaintenanceScreen` ile BİREBİR AYNI desen (Kural
 * 12) — yeni bir UI dili/mimari YOK. Error Code Standard (errors.<KOD>
 * çevirisi, ham hata ASLA gösterilmez) baştan uygulandı.
 *
 * NAVİGASYON: Bu ekran henüz hiçbir rotaya bağlı DEĞİL (Sprint 8.2
 * kapsamı dışında — Bakım'ın Sprint 5.2→5.3 aşamalı yaklaşımıyla
 * tutarlı, navigasyon Sprint 8.3'te).
 *
 * GLOBALIZATION POLICY: Hiçbir metin doğrudan yazılmaz.
 */

import { useEffect, useState } from "react";
import { useTranslation } from "react-i18next";
import { useHarvestRecords, type UseHarvestRecordsScope } from "./hooks/useHarvestRecords";
import { HarvestRecordList } from "./components/HarvestRecordList";
import { HarvestRecordForm } from "./HarvestRecordForm";
import { addBackButtonListener } from "../../native/appBackButton";
import type { HarvestRecord, NewHarvestRecordInput } from "./domain/harvest.types";

type HarvestView = { mode: "list" } | { mode: "create" } | { mode: "edit"; record: HarvestRecord };

interface HarvestScreenProps {
  scope: UseHarvestRecordsScope;
  /** `scope.mode === "tree"` ise zorunlu (formun `parcelId`'ye ihtiyacı var); `scope.mode === "parcel"` ise `scope.parcelId` ile aynı olmalı. */
  parcelId: string;
  onBack: () => void;
}

export function HarvestScreen({ scope, parcelId, onBack }: HarvestScreenProps) {
  const { t } = useTranslation();
  const {
    records,
    status,
    errorCode,
    hasMore,
    loadMore,
    createRecord,
    updateRecord,
    deactivateRecord,
  } = useHarvestRecords(scope);
  const [view, setView] = useState<HarvestView>({ mode: "list" });

  useEffect(() => {
    return addBackButtonListener(() => {
      if (view.mode !== "list") {
        setView({ mode: "list" });
      } else {
        onBack();
      }
    });
  }, [view, onBack]);

  const handleSelect = (record: HarvestRecord) => {
    setView({ mode: "edit", record });
  };

  const handleSubmit = async (input: NewHarvestRecordInput) => {
    if (view.mode === "edit") {
      await updateRecord(view.record.id, {
        harvestDate: input.harvestDate,
        quantityKg: input.quantityKg,
        notes: input.notes,
      });
    } else {
      await createRecord(input);
    }
    setView({ mode: "list" });
  };

  const handleDelete = async () => {
    if (view.mode !== "edit") return;
    await deactivateRecord(view.record.id);
    setView({ mode: "list" });
  };

  if (view.mode === "create" || view.mode === "edit") {
    const treeId = scope.mode === "tree" ? scope.treeId : null;
    return (
      <HarvestRecordForm
        parcelId={parcelId}
        treeId={view.mode === "edit" ? view.record.treeId : treeId}
        initialValue={view.mode === "edit" ? view.record : undefined}
        onSubmit={handleSubmit}
        onCancel={() => setView({ mode: "list" })}
        onDelete={view.mode === "edit" ? handleDelete : undefined}
      />
    );
  }

  return (
    <main className="status-screen">
      <h1 className="status-screen__title">{t("harvest.screenTitle")}</h1>

      <button type="button" className="lock-screen__button" onClick={() => setView({ mode: "create" })}>
        {t("harvest.addButton")}
      </button>

      {status === "idle" || (status === "loading" && records.length === 0) ? (
        <p className="status-card__value">{t("common.loading")}</p>
      ) : null}

      {status === "error" ? (
        <div className="status-card status-card--error">
          <p className="status-card__value">
            {t(`errors.${errorCode}`, { defaultValue: t("errors.SYS_001") })}
          </p>
        </div>
      ) : null}

      {status === "ready" && records.length === 0 ? (
        <p className="status-card__value">{t("harvest.emptyState")}</p>
      ) : null}

      {records.length > 0 ? (
        <>
          <HarvestRecordList records={records} onSelect={handleSelect} />
          {status === "loading" ? <p className="status-card__value">{t("common.loading")}</p> : null}
          {hasMore && status !== "loading" ? (
            <button type="button" className="lock-screen__button" onClick={loadMore}>
              {t("harvest.loadMoreButton")}
            </button>
          ) : null}
        </>
      ) : null}
    </main>
  );
}

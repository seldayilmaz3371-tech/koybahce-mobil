...
describe("TreesScreen", () => {
  it("Loading State: mount sonrası hemen yükleniyor mesajı gösterir", async () => {
    ...
    render(<TreesScreen mode={{ mode: "parcel", parcelId }} onBack={() => {}} onViewObservations={vi.fn()} />);
  ...
  it("Listeleme (Parcel Mode): sadece verilen parselin ağaçlarını gösterir", async () => {
    ...
    render(<TreesScreen mode={{ mode: "parcel", parcelId: parcelA }} onBack={() => {}} onViewObservations={vi.fn()} />);
  ...
  it("Navigation: 'Parsellere Dön' butonu onBack'i çağırır", async () => {
    const parcelId = await createTestParcel();
    const onBack = vi.fn();
    render(<TreesScreen mode={{ mode: "parcel", parcelId }} onBack={onBack} onViewObservations={vi.fn()} />);

...
describe("TreesScreen", () => {
  it("Loading State: mount sonrası hemen yükleniyor mesajı gösterir", async () => {
    ...
    render(<TreesScreen mode={{ mode: "parcel", parcelId }} onBack={() => {}} onViewObservations={vi.fn()} onViewMaintenance={vi.fn()} onViewHarvest={vi.fn()} onViewAiChat={vi.fn()} />);
  ...
  it("Error State: repository hatası ÇEVRİLMİŞ mesajla gösterilir, ham hata ASLA görünmez (Sprint 4.3.1)", async () => {
    ...
    render(<TreesScreen mode={{ mode: "parcel", parcelId: "herhangi-bir-id" }} onBack={() => {}} onViewObservations={vi.fn()} onViewMaintenance={vi.fn()} onViewHarvest={vi.fn()} onViewAiChat={vi.fn()} />);
  ...
  it("Listeleme (Parcel Mode): sadece verilen parselin ağaçlarını gösterir", async () => {
    ...
    render(<TreesScreen mode={{ mode: "parcel", parcelId: parcelA }} onBack={() => {}} onViewObservations={vi.fn()} onViewMaintenance={vi.fn()} onViewHarvest={vi.fn()} onViewAiChat={vi.fn()} />);
  ...
  it("Reference Mode görünümü: tüm parsellerdeki referans ağaçları gösterir, 'Ekle' butonu YOK", async () => {
    ...
    render(<TreesScreen mode={{ mode: "reference" }} onBack={() => {}} onViewObservations={vi.fn()} onViewMaintenance={vi.fn()} onViewHarvest={vi.fn()} onViewAiChat={vi.fn()} />);
  ...
  it("Navigation: 'Parsellere Dön' butonu onBack'i çağırır", async () => {
    const parcelId = await createTestParcel();
    const onBack = vi.fn();
    render(<TreesScreen mode={{ mode: "parcel", parcelId }} onBack={onBack} onViewObservations={vi.fn()} onViewMaintenance={vi.fn()} onViewHarvest={vi.fn()} onViewAiChat={vi.fn()} />);
...
describe("TreesScreen — Toplu Ağaç Oluşturma (Sprint 3.10)", () => {
  ...
  it("Madde 3 — canlı önizleme, başlangıç/adet girilirken güncellenir ('151 → 155 · Total: 5 Trees')", async () => {
    const parcelId = await createTestParcel();
    render(
      <TreesScreen mode={{ mode: "parcel", parcelId }} onBack={() => {}} onViewObservations={vi.fn()} onViewMaintenance={vi.fn()} onViewHarvest={vi.fn()} onViewAiChat={vi.fn()} />
    );
    await waitFor(() => expect(screen.getByText("Bulk Create")).toBeTruthy());
  ...
  it("bir ağacı düzenlerken 'View Maintenance' onViewMaintenance'i DOĞRU ağaçla çağırır", async () => {
    ...
    render(
      <TreesScreen
        ...
        onBack={() => {}}
        onViewObservations={vi.fn()}
        onViewMaintenance={onViewMaintenance} onViewHarvest={vi.fn()} onViewAiChat={vi.fn()}
      />
    );
